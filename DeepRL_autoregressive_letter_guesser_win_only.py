...
def create_model_id(epochs, actor_repetition, critic_repetition, actor_network_size, learning_rate, batch_size):
    ...
    return f"_FINAL_{timestamp}_ARLG-WR-win_only_epo-{epochs}_AR-{actor_repetition}_CR-{critic_repetition}_AS-{actor_network_size}-Lr-{learning_rate}-Bs-{batch_size}"
...
class Actor:
    ...
    def batch_update_with_position_rewards(self, states, actions, rewards, position_rewards, next_states, old_position_probs, dones):
        ...
        if random and batch_size > self.sample_size:
            ...
            #Filter old_position_probs to use only the sampled indices.
            batch_old_probs = [old_position_probs[i] for i in indices.tolist()]
        else:
            ...
            batch_dones = dones
            batch_old_probs = old_position_probs
        ...
        for position in range(self.env.word_length):
            ...
            for _ in range(self.actor_repetition):
                ...
                for pos in range(self.env.word_length):
                    # Create input data for current position
                    if pos == 0:
                        in_data = batch_states
                    else:
                        # Get previous letter indices for all samples
                        prev_letters = all_letter_indices[:, :pos]

                        # Create one-hot encodings using F.one_hot (more efficient)
                        prev_onehot_3d = F.one_hot(prev_letters, num_classes=26).float()
                        # Reshape to [batch_size, pos*26]
                        prev_onehot = prev_onehot_3d.reshape(mini_batch_size, -1)
                    ...
                    # Get probabilities of selected letters
                    ...
                    # Stack old probabilities more efficiently
                    ...
                    # Calculate importance ratio for this position
                    ratio = selected_letter_probs_inner / old_position_specific_probs_inner
...
A = Actor(env, batch_size=5000, epsilon=0.1, learning_rate=1e-5, actor_repetition=10, critic_repetition=2,
          random_batch=False, sample_size=1000, display_progress_bar=False)
...
A.train(epochs=400000, print_freq=5000, prune=False, display_progress_bar=False)
